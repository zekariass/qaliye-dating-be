-- ============================================================
-- QALIYE SUPPORT CHAT — IMPROVED PRODUCTION BASELINE
-- PostgreSQL / Supabase
--
-- Design:
--   * One persistent support conversation per application user
--   * One IDLE conversation is created automatically with each app user
--   * Public messages and internal staff notes are stored separately
--   * Public message sequences therefore contain no hidden gaps
--   * ADMIN and MODERATOR roles are authorized as support staff
--   * All mutations go through SECURITY DEFINER functions
--   * Message + attachment metadata is committed atomically
--   * Storage objects must be uploaded to a private bucket first
--   * The backend should remove orphaned uploads when an RPC fails
--
-- Assumptions:
--   * public.app_users(id UUID, role ...) already exists
--   * app_users.role uses ADMIN and MODERATOR for support staff
--   * Supabase roles anon, authenticated, and service_role exist
--   * The Spring backend uses service_role or replaces the grants below
--     with a dedicated database role
-- ============================================================
...
CREATE TABLE public.support_conversations (
    ...
    -- Assignment functions ensure this user has ADMIN or MODERATOR role.
    assigned_staff_user_id          UUID
        REFERENCES public.app_users(id) ON DELETE SET NULL,

    -- Sequence assigned to the next PUBLIC message.
    -- Internal notes use a separate table and do not consume this sequence.
    next_public_sequence            BIGINT NOT NULL DEFAULT 1
        CHECK (next_public_sequence > 0),

    -- General public-message watermarks.
    -- A cursor may point at a message from either sender; unread queries
    -- should additionally filter by sender_type.
    ...
    -- Set when the conversation entered WAITING_STAFF.
    -- User follow-up messages do not reset it.
    waiting_since                   TIMESTAMPTZ,

    first_staff_response_at         TIMESTAMPTZ,

    -- Updated by both public messages and internal notes.
    -- This column is intentionally not granted to authenticated clients.
...
-- The actual customer-waiting queue.
...
CREATE TABLE public.support_messages (
    ...
    -- NULL is permitted only for attachment-only messages created
    -- through the append RPCs.
    body                        TEXT,

    client_message_id           UUID NOT NULL,

    -- SHA-256 of the normalized idempotent request.
    request_hash                CHAR(64) NOT NULL
        CHECK (request_hash ~ '^[0-9a-f]{64}$'),

    -- Backend-only data. This column is not granted to clients.
...
-- The UNIQUE(conversation_id, sequence_number) index supports both
-- forward and backward conversation pagination.
...
CREATE TABLE public.support_attachments (
    ...
    -- Change this constraint if a different private bucket name is used.
    ...
    file_size_bytes             BIGINT NOT NULL
        CHECK (
            file_size_bytes > 0
            AND file_size_bytes <= 26214400 -- 25 MiB
        ),
...
BEGIN
    ...
    -- The conversation must already have been provisioned by the
    -- app_users AFTER INSERT trigger.
    SELECT *
    ...
    -- Attachment metadata is part of this same database transaction.
...
REVOKE ALL
ON FUNCTION public.append_support_user_message(
    UUID,
    UUID,
    TEXT,
    JSONB,
...
BEGIN
    ...
    -- Check idempotency before current-state validation so a retry of an
    -- already committed message still succeeds after the conversation closes.
    SELECT *
    INTO inserted_message
    FROM public.support_messages AS m
...
REVOKE ALL
ON FUNCTION public.append_support_staff_message(
    UUID,
    UUID,
    UUID,
    TEXT,
...
REVOKE ALL
ON FUNCTION public.append_support_internal_note(
    UUID,
    UUID,
    UUID,
    TEXT,
...
REVOKE ALL
ON FUNCTION public.mark_support_conversation_read_by_user(
    UUID,
    UUID,
    BIGINT
)
...
BEGIN
    ...
    -- Highest public-message sequence read by any staff member.
    UPDATE public.support_conversations
...
REVOKE ALL
ON FUNCTION public.mark_support_conversation_read_by_staff(
    UUID,
    UUID,
    BIGINT
)
...
CREATE OR REPLACE FUNCTION public.close_support_conversation_by_system(
    p_conversation_id UUID
)
RETURNS VOID
LANGUAGE plpgsql
...
REVOKE ALL
ON FUNCTION public.close_support_conversation_by_system(UUID)
FROM PUBLIC, anon, authenticated;
...
ALTER TABLE public.support_conversation_staff_reads
ENABLE ROW LEVEL SECURITY;


-- Users may read only their own conversation.
...
-- All rows in support_messages are public messages.
...
-- Users may read only metadata for attachments on their messages.
-- The actual Storage bucket remains private; issue short-lived signed URLs
-- from the backend after performing the same authorization check.
...
USING (
    EXISTS (
        SELECT 1
        FROM public.support_messages AS m
        JOIN public.support_conversations AS c
...
-- Remove broad table access first.
REVOKE ALL
ON public.support_conversations
...
-- Grant only user-safe conversation columns.
GRANT SELECT (
    id,
    user_id,
    status,
    ...
    last_public_message_sender_type,
    waiting_since,
    closed_at,
    created_at
)
...
-- Do not expose sender_user_id, client_message_id, request_hash, or metadata.
GRANT SELECT (
    id,
    conversation_id,
    sequence_number,
...
-- Do not expose storage_bucket or storage_path.
GRANT SELECT (
    id,
    message_id,
    file_name,
...
-- ============================================================
-- 21. STORAGE SECURITY REQUIREMENTS
-- ============================================================
 ...
 *     support-attachments
 *
 * Do not create a broad authenticated SELECT policy on storage.objects.
 *
 * Recommended download flow:
 *   1. Client asks Spring backend for an attachment URL.
 *   2. Backend verifies ownership through support_attachments ->
 *      support_messages -> support_conversations.
 *   3. Backend creates a short-lived signed URL.
 *
 * Recommended upload flow:
 *   1. Backend generates a non-guessable storage path.
 *   2. Upload to the private bucket.
 *   3. Call append_support_user_message or append_support_staff_message
 *      with the uploaded object's metadata.
 *   4. If the RPC fails, remove the orphaned Storage object.
 *
 * A database DELETE does not automatically delete a Storage object.
 * Use the Supabase Storage API for object cleanup.
 */


-- ============================================================
-- 22. EXAMPLE BACKEND QUERIES
-- ============================================================
...
SELECT
    ...
    c.last_public_message_at
FROM public.support_conversations AS c
WHERE c.status = 'WAITING_STAFF'
ORDER BY
    c.priority ASC,
    c.waiting_since ASC,
    c.id;
...
WHERE m.conversation_id = :conversation_id
  AND m.sequence_number < :before_sequence
ORDER BY m.sequence_number DESC
LIMIT :page_size;


-- Customer unread staff messages:
SELECT count(*)
FROM public.support_messages AS m
JOIN public.support_conversations AS c
  ON c.id = m.conversation_id
WHERE c.user_id = :user_id
  ...
  AND m.sequence_number > c.user_last_read_sequence;


-- Staff unread user messages for a specific agent:
SELECT count(*)
FROM public.support_messages AS m
LEFT JOIN public.support_conversation_staff_reads AS r
  ON r.conversation_id = m.conversation_id
 AND r.staff_user_id = :staff_user_id
WHERE m.conversation_id = :conversation_id
...
-- ============================================================
-- 23. BACKFILL DEFAULT CONVERSATIONS FOR EXISTING USERS
-- ============================================================
...
-- ============================================================
-- 24. IMPORTANT DEPLOYMENT NOTES
-- ============================================================

/*
 * 1. The provisioning trigger currently creates one IDLE support
 *    conversation for every row inserted into public.app_users. If that
 *    table also contains system accounts that should not receive a support
 *    conversation, add an account-type condition to the trigger function.
 *
 * 2. Staff authorization comes from public.app_users.role. Only users
 *    whose role is ADMIN or MODERATOR may perform staff operations.
 *    Changing the role immediately changes support access.
 *
 * 3. If Spring uses a dedicated DB role instead of service_role:
 *      - replace every GRANT EXECUTE ... TO service_role
 *      - grant that role only the required RPC execution privileges
 *
 * 4. Do not allow application code to INSERT directly into support tables.
 *
 * 5. Consider owning SECURITY DEFINER functions with a dedicated NOLOGIN
 *    role and protecting schema CREATE privileges.
 *
 * 6. Add rate limiting, malware scanning, content-type allowlists,
 *    retention rules, and audit logging in the backend.
 *
 * 7. If public message editing is needed later, keep support_messages
 *    immutable and add a separate message revision/audit table.
 */